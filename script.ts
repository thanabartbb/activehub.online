...
function switchScreen(screenId: string): void {
  const allScreens = document.querySelectorAll<HTMLElement>('.screen-view');
...
function updateBottomNavTabs(screenId: string): void {
...
function setProjectFilter(btn: HTMLElement, filterType: string): void {
  document.querySelectorAll<HTMLElement>('.project-filter-pill').forEach(pill => {
...
function selectService(name: string, endpoint: string, type: string, desc: string, dotColor: string): void {
  ...
  if (runTextEl) runTextEl.textContent = 'Run ' + name + ' Agent';
...
function simulateConnect(provider: string): void {
  hideConnectModal();
...
function simulateLogin(provider: string): void {
  hideLoginModal();
...
function closeRightDrawer(event?: Event): void {
  if (event && event.target && (event.target as HTMLElement).id !== 'right-drawer-overlay') return;
  const overlay = document.getElementById('right-drawer-overlay');
  const panel = document.getElementById('right-drawer-panel');
  if (!overlay || !panel) return;
...
function handleDrawerServiceClick(name: string, endpoint: string): void {
  closeRightDrawer();
